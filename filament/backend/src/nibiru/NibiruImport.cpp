...
API_NibiruGetEyeProjection NibiruImport::pAPI_NibiruGetEyeProjection = nullptr;
...
bool NibiruImport::Init()
{
	...
	if (GSharedLibraryHandle != NULL)
	{
		...
		if (!pAPI_NibiruInit) 
			UE_LOG("API NibiruInit not found!");
		...
		if (!pAPI_NibiruGetFloatValue) 
			UE_LOG("API NibiruGetFloatValue not found!");
		...
		if (!pAPI_NibiruComputeDistortedPoint) 
			UE_LOG("API NibiruComputeDistortedPoint not found!");
		...
		// 手柄接口
		pAPI_NibiruGetEyeProjection = (API_NibiruGetEyeProjection) dlsym(GSharedLibraryHandle, "NibiruGetEyeProjection");

        UE_LOG("Nibiru Import Load V2 Successs.");
		return true;
	} else {
		UE_LOG("Nibiru Import Load V2 Failed.");
	}
	return false;
}
...
{
	 if(GSharedLibraryHandle)
     {
	     dlclose(GSharedLibraryHandle);
         GSharedLibraryHandle = nullptr;
     }
}
...
// 手柄接口
void NibiruImport::NibiruGetEyeProjection(float* projL, float* projR)
{
	if(NibiruImport::pAPI_NibiruGetEyeProjection)
	{
       NibiruImport::pAPI_NibiruGetEyeProjection(projL, projR);
	}
}
//===================== APIS =====================